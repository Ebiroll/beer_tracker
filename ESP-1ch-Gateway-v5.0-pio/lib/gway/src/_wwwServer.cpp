...
// ----------------------------------------------------------------------------
void stringTime(unsigned long t, String& response) {
...
// ----------------------------------------------------------------------------
void setVariables(const char *cmd, const char *arg) {
	...
	if (strcmp(cmd, "DEBUG")==0) {									// Set debug level 0-2
		...
		writeGwayCfg(CONFIGFILE, WiFi.SSID());									// Save configuration to file
	}
	...
	if (strcmp(cmd, "FCNT")==0)   {
		frameCount=0;
		rxLoraModem();												// Reset the radion with the new frequency
		writeGwayCfg(CONFIGFILE, WiFi.SSID());
	}
	#endif
